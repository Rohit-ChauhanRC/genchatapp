...
class ChatBackupController extends GetxController {
  ...
  Future<void> loadBackupInfo() async {
    ...
    if (exists) {
      ...
      lastBackupTime.value = DateFormat(
        'yyyy-MM-dd – hh:mm a',
      ).format(lastModified);
    } else {
      lastBackupTime.value = "No backup found";
    ...
    if (await backupDir.exists()) {
      ...
      debugPrint('Backup size: $sizeInMB MB');
  ...
  Future<void> createBackup() async {
    try {
      ...
      dbService.setUserId(
        sharedPref.getString(UserDefaultsKeys.backupUserId).toString(),
      );

      await dbService.backupAllData(
        onProgress: (done, total, uploadedBytes, totalBytes) {
          copiedFiles.value = done;
          totalFiles.value = total;
          backupProgress.value = done / total;
          uploadedSizeMB.value =
              (uploadedBytes / (1024 * 1024)).toStringAsFixed(2) + ' MB';
          totalSizeMB.value =
